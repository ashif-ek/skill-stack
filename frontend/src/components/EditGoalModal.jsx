...
export default function EditGoalModal({ goal, onClose, onUpdated }) {
  ...
  useEffect(() => {
    if (goal) {
      setForm({
        ...
        resource_type: goal.resource_type || "course",
        platform: goal.platform || "",
        resource_url: goal.resource_url || "",
        status: goal.status || "started",
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError("Could not update the learning goal.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 p-4 transition-opacity">
      <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl border border-slate-200 max-h-[90vh] overflow-y-auto">
        <div className="mb-6 flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Edit Learning Goal</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700">Skill Name</label>
            <input
              name="skill_name"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 shadow-sm bg-white"
              >
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700">Resource URL (Optional)</label>
            <input
              name="resource_url"
